...
class Sarie {
  ...
  async registerModels() {
    ...
    let { defaultDatabase, databases } = this.appConfig;
    if (!databases || this.databases.length === 0) {
      logger.warn(`starting without any database`);
      return;
    }
    if (!defaultDatabase) {
      logger.warn(`default database name not found. Using the first one ${databases[0].name}`);
    ...
    for (const modelFile of modelSchemas) {
      if (path.basename(modelFile) === 'index.js') continue;
      const model = require(modelFile);
      const modelConn = (this.databases[model.dbName] || this.databases[defaultDatabase])
        .model(model.collectionName, model.schema);
      model.Model = modelConn;
      global[sanitizeFileName(model.collectionName)] = modelConn;
    }
  }
  ...
  async registerRoutes() {
    const commonMiddlewaresPath = path.resolve(this.dirPath, 'middlewares/*.js');
    const commonMiddlewaresNames = await globPromise(commonMiddlewaresPath);
    const commonMiddlewares = {};
    commonMiddlewaresNames.forEach(name => {
      commonMiddlewares[`${path.parse(name).name}`] = require(name);
    });
        ...
        if (endpoints) {
          for (const endpoint in endpoints) {
            ...
            const middlewares = middlewareNames.map((name) => {
              ...
              try {
                const mid = require(namePath);
                return mid;
              } catch (err) {
                if (commonMiddlewares[name]) return commonMiddlewares[name];
                else {
                  logger.error(`unknown middleware name: ${name} for endpoint ${endpoint}`);
                  process.exit(0)
                }
              }
            });
